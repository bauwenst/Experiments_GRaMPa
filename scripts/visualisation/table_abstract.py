...
class Parser(ABC, Generic[Instance,RawRowKeys,RawColKeys,SortableRowKeys,SortableColKeys]):
    """
    Provides an implicit, subtractive approach to turning a set of results into a table.
    Basically: filter out all the results you don't want, sort them, and add them to a table. That table cannot be any
    other table than the table you want. The alternative approach would be additive, where you select the results you
    do want by defining just literally the table's hierarchy using dictionaries, only every querying what the explicitly
    known table structure requires.
    """
...
class CsvParser(Parser[dict,RawRowKeys,RawColKeys,SortableRowKeys,SortableColKeys]):
    def _generateInstances(self, path: Path) -> Iterator[Instance]:
